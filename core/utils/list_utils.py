...
def check_level_indent_exact(
    ...
    for idx, para in enumerate(paragraphs):
        ...
        if lf.ListLevelNumber == level:
            left_indent = para.LeftIndent # main text starting point, excluding bullet
            first_line_indent = left_indent + para.FirstLineIndent # bullet starting point, normally hanging indent, should be a negative value

            if not (isclose(first_line_indent, align, abs_tol=0.1) and 
                    isclose(left_indent, indent, abs_tol=0.1)):
                return False
    return True